...
export default function DialogueChallenge({ game, onBack }: Props) {
            ...
            <div style={{ minHeight: '100vh', background: '#0a0014', color: 'white', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
                <div style={{ background: 'rgba(20,0,10,0.8)', padding: 40, borderRadius: 24, border: '1px solid rgba(251,146,60,0.3)', maxWidth: 500, width: '100%', textAlign: 'center' }}>
                    {submitting ? (
                        ...
                        <>
                            <h1 style={{ fontSize: 48, fontWeight: 900, color: '#10b981', marginBottom: 16 }}>Challenge Complete!</h1>
    ...
    return (
        <div style={{ minHeight: '100vh', background: '#080010', color: 'white', display: 'flex', flexDirection: 'column', fontFamily: "'Outfit', sans-serif" }}>
            {/* Header Bar */}
            <div style={{ padding: '20px 40px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid rgba(255,255,255,0.05)', background: 'rgba(255,255,255,0.02)' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
                    ...
                    <div>
                        ...
                        <div style={{ fontSize: 13, color: 'rgba(255,255,255,0.5)', fontWeight: 600 }}>Quote {currentIdx + 1} of {game.questions.length}</div>
                    </div>
                </div>
            ...
            <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24 }}>
                <div style={{ maxWidth: 800, width: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                    ...
                    <div style={{ fontSize: 48, color: '#a855f7', opacity: 0.5, marginBottom: -20, alignSelf: 'flex-start', marginLeft: 20 }}>"</div>
                    <div style={{
                        padding: '40px', background: 'linear-gradient(145deg, rgba(168,85,247,0.1), rgba(0,0,0,0))',
                        borderRadius: 24, border: '1px solid rgba(168,85,247,0.2)', width: '100%',
                        boxShadow: '0 10px 30px rgba(0,0,0,0.5), inset 0 0 20px rgba(168,85,247,0.05)',
                        marginBottom: 40
                    }}>
                        <h1 style={{ fontSize: 32, fontWeight: 700, textAlign: 'center', lineHeight: 1.5, color: '#f1f5f9', fontStyle: 'italic' }}>
                            {currentQ?.questionText || 'Loading quote...'}
                        </h1>
                    </div>
                    <div style={{ fontSize: 48, color: '#a855f7', opacity: 0.5, marginTop: -60, alignSelf: 'flex-end', marginRight: 20 }}>"</div>

                    <div style={{ marginBottom: 24, fontSize: 16, fontWeight: 600, color: 'rgba(255,255,255,0.5)', textTransform: 'uppercase', letterSpacing: 2 }}>Who said this? / From which movie?</div>

                    <div style={{ display: 'grid', gridTemplateColumns: '1fr', gap: 16, width: '100%', maxWidth: 500 }}>
                        {currentQ?.options.map((opt: string, i: number) => {
                            ...
                            return (
                                <button
                                    ...
                                    style={{
                                        padding: '20px 24px',
                                        background: isSelected ? 'linear-gradient(135deg, #8b5cf6, #a855f7)' : 'rgba(255,255,255,0.05)',
                                        ...
                                        boxShadow: isSelected ? '0 10px 20px rgba(168,85,247,0.3)' : 'none',
                                    }}
                                    onMouseEnter={e => {
                                        if (!selectedOpt) {
